...
final class SignInServiceTests: XCTestCase {
    ...
    func testIsSignedIn_whenRootURLIsEqualToNil_shouldReturnFalse() {
        ...
        let service = SignInService(keychain: keychain, networking: makeNetworking(), cookieStorage: CookieStorageMock(), pinStorage: PinStorageMock(), feedStorage: FeedStorageMock())
    ...
    func testSignIn_whenSignInHTMLDoesNotHaveAuthenticityToken_shouldThrowNoAuthenticityTokenError() async throws {
        ...
        let service = SignInService(keychain: KeychainMock(), networking: networking, cookieStorage: CookieStorageMock(), pinStorage: PinStorageMock(), feedStorage: FeedStorageMock())
    ...
    func testSignIn_whenSignInSucceeds_shouldKeychainHaveApiKeyAndCookie() async throws {
        ...
        let service = SignInService(keychain: keychain, networking: networking, cookieStorage: CookieStorageMock(), pinStorage: PinStorageMock(), feedStorage: FeedStorageMock())
        XCTAssertNil(keychain.apiKey)
        XCTAssertNil(keychain.cookie)
        try await service.signIn(username: "username", password: "password")
